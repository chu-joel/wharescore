...
| `market.market_heat, .trend.cagr_*` | MarketSection | "Affordable?" | No |
...
| `report.liveability` (crime, deprivation) | HostedNeighbourhoodStats | Yes |
| `nearby_doc` | HostedOutdoorRec | Yes |
| `report.planning` | HostedInfrastructure | Yes |
| `report.property, report.hazards` | HostedHealthyHomes | Yes (renter only) |
...
| `school_zones` | Array {school_name, school_id, institution_type} | school_zones table | HostedSchoolZones |
| `road_noise` | {laeq24h: number} | noise_contours table | HostedRoadNoise |
| `terrain` | {elevation_m, slope_degrees, aspect_label, aspect_degrees, ruggedness} | SRTM raster via `walking_isochrone.py` | HostedTerrain |
...
| downloadGateStore | `stores/downloadGateStore.ts` | isAuthenticated, credits {plan, creditsRemaining, dailyLimit, downloadsToday}, showUpgradeModal | No | No (reads from /account/credits) |
| pdfExportStore | `stores/pdfExportStore.ts` | isGenerating, downloadUrl, shareUrl, addressId, persona, error | No | Yes (triggers export API) |
...
|--------|------|:----:|------|---------|-------------------|
| GET | `/property/{id}/report` | No | 20/min | Full property report | get_property_report() SQL, _fix_unit_cv(), _overlay_transit_data(), _overlay_terrain_data() |
...
| GET | `/property/{id}/rates` | No | 10/min | Live council rates | 25 council APIs, council_valuations |
...
| POST | `/property/{id}/export/pdf/start` | Yes | 20/hr | Start PDF gen (deducts credit) | _generate_pdf_background() |
...
| GET | `/report/{token}` | No | 30/min | Hosted report snapshot | report_snapshots |
...
| GET | `/account/credits` | Yes | — | Plan + credit balance | users, report_credits |
...
| POST | `/account/redeem-promo` | Yes | 15/min | Redeem promo code | promo_redemptions, report_credits |
| POST | `/checkout/session` | Yes | 15/min | Stripe checkout (auth) | Stripe API |
| POST | `/checkout/guest-session` | No | 5/min | Stripe checkout (guest) | guest_purchases, Stripe API |
