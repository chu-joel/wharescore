...
| GET | `/admin/analytics/overview` | Admin | 30/min | Analytics dashboard: today stats, trends, top endpoints, slow requests, errors, **unique visitors (DAU/WAU/MAU + new/returning today)**, **conversion funnel (7 day, visit → search → report view → generated → payment, by distinct ip_hash)** | app_events, perf_metrics, error_log, daily_metrics |
