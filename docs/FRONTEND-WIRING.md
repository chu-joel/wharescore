...
| `coastal` | {tier, ground_elevation_m, coast_distance_m, storm_tide_100yr_distance_m, vlm_mm_yr, scenarios[], headline, narrative, narrative_renter?, score_impact} or null | `services/coastal_timeline.py::build_coastal_exposure(report)`. MVP uses existing hazard flags + terrain elevation + NZ SeaRise national-average SLR scenarios (NATIONAL_SLR constant). Per-point SeaRise data + LINZ coastline + NIWA storm-tide polygons not yet loaded: `coast_distance_m`, `storm_tide_100yr_distance_m`, `vlm_mm_yr` are null placeholders. `score_impact.delta` halved when a council coastal layer already fires (prevents double-counting against `risk_score.py`). Not yet fed into `risk_score.py` itself — currently cosmetic chip only. | HostedCoastalTimeline |
