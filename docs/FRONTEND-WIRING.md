...
| Chip UI | `frontend/src/components/property/PropertyDetailsChip.tsx` | Pill row with click-to-edit dropdowns + inline rent (renter) / asking-price (buyer) input that writes to `useRentInputStore.weeklyRent` / `useBuyerInputStore.askingPrice`. Desktop: `lg:sticky lg:top-14` inside the report column with `lg:flex lg:justify-end` so it right-aligns to ~260px, never overlaps the PersonaToggle (which is `sticky top-0` in the same scroll container). Mobile/tablet: inline above PropertySummaryCard, scrolls with content (every fixed bottom corner is booked). Three layout instances are mounted (one per breakpoint, only one visible) — `lastAutoSaveKey` is module-scoped so they share one dedupe key. Auto-saves to `/api/v1/budget-inputs` (debounced 800ms) when bedrooms AND bathrooms are both set, so descriptors enrich the same upserted row that PriceAdvisorCard / RentComparisonFlow / BuyerBudgetCalculator hit. Bedrooms + bathrooms + rent/price intentionally not seeded — they pulse red until the user picks them so the Critical Renter / Buyer rules fire on real intent. |
