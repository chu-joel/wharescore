...
**Performance note:** Report endpoint has `[PERF]` timing logs via `print()`. Cold cache ~1s after index fix (was 9-12s). Migration 0031 adds index on `addresses(gd2000_xcoord, gd2000_ycoord)`. Quick Report snapshots skip Valhalla terrain (~60s saving) — terrain backfilled async via `_backfill_terrain()` and merged into snapshot JSONB.
