...
**Performance note:** Report endpoint has `[PERF]` timing logs. Cold cache: SQL ~1-2s, overlays ~8-10s. The `get_property_report()` SQL is fast; the bottleneck is Python overlay functions (transit, event history, terrain).
