...
| — (UI layout) | QuestionAccordion | 5b. DEEP DIVE — `DEFAULT_EXPANDED = []`. All question panels (Safety/deal-breakers, rent-fair, true-cost, daily-life, neighbourhood, restrictions, investment) start collapsed for both personas so the page reads as Snapshot → Findings → Action → Upgrade → (compact) deep-dive menu. Readers drill in per section. | No |
