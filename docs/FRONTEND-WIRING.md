...
|--------|------|:----:|------|---------|-------------------|
| GET | `/property/{id}/report` | No | 20/min | Full property report. `?fast=true` skips Valhalla terrain (~5-15s) for progressive loading — frontend fetches fast first (deferred), then full in background | get_property_report() SQL, _overlay_transit_data() (also fixes nearest_train for non-Wellington), _overlay_event_history(), _overlay_terrain_data() (skipped when fast=true) |
...
| POST | `/report/{token}/upgrade` | Optional | 10/min | Upgrade Quick→Full ($9.99), returns Stripe checkout_url. Uses STRIPE_PRICE_UPGRADE or STRIPE_PRICE_FULL_SINGLE. | report_snapshots, Stripe |
...
| POST | `/checkout/session` | Yes | 15/min | Stripe checkout (auth). Plans: quick_single ($4.99), full_single ($9.99), pro ($140/mo) | Stripe API |
| POST | `/checkout/guest-session` | No | 5/min | Stripe checkout (guest). Plans: quick_single, full_single | guest_purchases, Stripe API |
