...
**Performance note:** Report endpoint has `[PERF]` timing logs via `print()` (not logger — logger output not captured in Docker). Cold cache: SQL ~1-2s, overlays ~8-10s. The `get_property_report()` SQL is fast; bottleneck is Python overlay functions.
