...
**Performance note:** Report endpoint has `[PERF]` timing logs via `print()`. Cold cache breakdown: SQL ~0.8s, detect_property_type ~0.1s (after index fix, was 8s), overlays ~0.2s. Migration 0031 adds the missing index on `addresses(gd2000_xcoord, gd2000_ycoord)` for the unit count query.
