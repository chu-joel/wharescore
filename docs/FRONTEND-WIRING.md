...
**Performance note:** Report endpoint has `[PERF]` timing logs via `print()`. Cold cache breakdown: SQL ~0.8s, detection+area_profile ~10s (bottleneck), overlays ~0.2s. The `detect_property_type()` function is the primary bottleneck.
