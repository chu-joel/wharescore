...
| Layer | File | Notes |
|---|---|---|
| Persisted store | `frontend/src/stores/propertyDetailsStore.ts` | `dwellingType, bedrooms, bathrooms, finishTier, hasParking`. Tracks `lastAddressId` so the chip can call `resetForAddress(newId)` on navigation, wiping per-property values. Persists choices for the SAME property across refreshes (lastAddressId is also persisted). `chipDismissed` is intentionally NOT persisted. `hydrateDefaults()` only fills unset values so user choices stick. |
...
| Chip UI | `frontend/src/components/property/PropertyDetailsChip.tsx` | Pill row with click-to-edit dropdowns + inline rent (renter) / asking-price (buyer) input that writes to `useRentInputStore.weeklyRent` / `useBuyerInputStore.askingPrice`. Desktop: `lg:sticky lg:top-14` inside the report column with `lg:flex lg:justify-end` so it right-aligns to ~260px, never overlaps the PersonaToggle (which is `sticky top-0` in the same scroll container). Mobile/tablet: inline above PropertySummaryCard, scrolls with content (every fixed bottom corner is booked). Three layout instances are mounted (one per breakpoint, only one visible) — `lastAutoSaveKey` and `lastAutoExpandKey` are module-scoped so they share one dedupe key. Auto-saves to `/api/v1/budget-inputs` (debounced 800ms) when bedrooms AND bathrooms are both set; auto-expands the rent-fair / true-cost accordion + dispatches `wharescore:highlight-{rent,price}-inputs` when the chip is fully filled (bedrooms + bathrooms + dwelling + finish + rent/price), so the advisor result is visible without hunting. Bedrooms + bathrooms + rent/price intentionally not seeded — they pulse red until the user picks them so the Critical Renter / Buyer rules fire on real intent. |
...
| Sub-flow integration | `RentComparisonFlow.tsx`, `PriceAdvisorCard.tsx`, `RentAdvisorCard.tsx` | Read shared store first, then their local fallback. The chip fans every typology change (dwelling, bedrooms, bathrooms, finish) to **all three** stores at once — `propertyDetailsStore` (shared), `useRentInputStore` (drives RentAdvisorCard / RentComparisonFlow), `useBuyerInputStore` (drives PriceAdvisorCard) — so swapping persona never loses or stales typology. Mirror useEffects also push persisted shared values into both per-persona stores on mount, so a user landing on a property where typology was filled in a previous session sees the rent / price advisor populated without touching the chip. Studio in RentComparisonFlow maps to '1' bed when mirrored (MBIE bonds have no studio category). RentComparisonFlow seeds its local `rentInput` from `useRentInputStore.weeklyRent` and mirrors store updates so rent typed into the chip flows into the auto-fetch (`/api/v1/property/{id}/market`). Both `RentAdvisorCard` and `PriceAdvisorCard` expose `wharescore:run-rent-advisor` / `wharescore:run-price-advisor` window events that the chip dispatches when the persona's input set becomes complete; each listener ref-binds to the freshest `handleAnalyse` closure so the estimate runs without a button click. Module-scoped `{rent,price}AdvisorRunInFlight` booleans dedupe the listeners across the 3 PropertyReport breakpoint mounts so only one POST fires per chip-complete event (cleared on the next macrotask). |
...
**Public banner** (`components/common/PublicBanner.tsx`, mounted in `app/layout.tsx`) — site-wide announcement bar fixed at the top of every page. Polls `GET /api/v1/public/banner` via `usePublicBanner` hook (60s staleTime). Renders only when an active banner exists in `admin_content` (managed at `/admin/content` via `ContentPanel.tsx` → `Save Banner`/`Clear`). Three styles via `type` (info/warning/success). Per-visitor dismissal stored in `localStorage` under `ws-banner-dismissed:{hash(text)}` so editing the text re-shows the banner for everyone, but the same text isn't shown twice. Exposes its measured height as the `--ws-banner-h` CSS variable on the document root; downstream chrome (`AppHeader`, full-viewport map containers, all `pt-14` pages) reads `var(--ws-banner-h,0px)` to stack correctly. Signed-in users see an inline **Buy Full Report — $2.99** CTA that POSTs `/api/v1/checkout/session` with `plan='full_single'` (no address_id — buys a credit, redeemable on any property) and redirects straight to Stripe.
...
| `coverage.available, .total, .per_category, .bonus_features` | DataLayersAccordion | 6. Below fold (compact mode). Indicator labels for dropped scoring indicators (`zone_permissiveness`, `height_limit`, `school_zone`) removed 2026-05-07 — those indicators no longer feed the composite, so the labels never render. The corresponding planning fields (`zone_name`, `max_height_m`, `school_zone`) are still rendered as informational context by PlanningSection / BuyerSnapshot / QuestionSummary. | No |
...
`HostedReportShell` no longer renders the centered text-only cover. Both Quick and Full hosted reports now show `HostedHeroCardNew` (in `components/new/sections/`) which mirrors the A8-real-snapshot design: poster-style 2-col card with eyebrow + title + sub + score ring + persona/coverage line on the left, and an 8-cell stat grid + property image on the right. Property image renders LINZ Basemaps aerial tiles directly via `<img>` (NZ-native, free, key already in env as `NEXT_PUBLIC_LINZ_API_KEY`) — a 2x2 grid of tiles at zoom 17 is positioned so the property pin lands at viewport centre. Hero collapses to a single column below 800 px via `.ws-hero-grid` rule in `tokens-new.css`. Stat grid pulls Title / Title # / Footprint / Land value / CV / CV date / Council / Multi-unit from `snapshot.report.property` + `report_detection`.
...
| POST | `/api/v1/admin/data-sources/refresh-due?limit=N&dry_run=bool` | Admin OR `Authorization: Bearer ADMIN_API_TOKEN` | 1/hour | Walks due sources, polls upstream metadata via `change_detection`, runs `run_loader()` only when changed. Validation gate active (rejects new row count < 50% of previous). `limit` caps the run (default 10) so backlogs don't run away. Single-flight via Redis — but **unlike** the other admin loader endpoints, only blocks when an active job has `status: "running"` (terminal entries from completed jobs do NOT block, so back-to-back cron runs work even within the 600s terminal-state TTL — verified d276942). **Response shape splits on `dry_run`:** `dry_run=true` is synchronous (cheap metadata polls only, returns `.results[]` in seconds — used by ops to verify classifications). `dry_run=false` is **fire-and-forget** — kicks off a background task and returns `{job_id, due_count, due_keys}` immediately so nginx doesn't 504 on long bulk reloads (auckland_flood is ~5 min). Per-source outcomes for the async path land in `data_source_health`; live progress in Redis `data_loader:active` (poll `GET /admin/data-sources/job`). Called by the daily GH Actions `data-refresh.yml` workflow which tolerates 409 as an expected "skipped" outcome. | DATA_SOURCES, data_source_health |
