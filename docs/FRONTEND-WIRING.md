...
|--------|------|:----:|------|---------|-------------------|
| GET | `/property/{id}/report` | No | 20/min | Full property report. `?fast=true` skips Valhalla terrain (~5-15s) for progressive loading — frontend fetches fast first, then full in background | get_property_report() SQL, _overlay_transit_data(), _overlay_event_history(), _overlay_terrain_data() (skipped when fast=true) |
