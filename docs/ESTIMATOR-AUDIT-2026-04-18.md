...
| 2 | `cv_uncertainty` in rent advisor | `price_advisor:cv_uncertainty(cv_age_months)` | **PARTIALLY IMPLEMENTED** `rent_advisor.py:1328-1345` | `cv_age_months` now computed and passed into `market_confidence_stars`. `cv_uncertainty()` itself still unused in rent advisor — dampening the quality-per-room adjustment magnitude on stale CVs is still a separate step. | Med — mostly better confidence score, not shift in band centre | S | P0 — stars done, band-centre dampening TBD |
