...
| E | Live MAPE backtest (validation plan steps 1–4) requires Postgres access. Not runnable from this local session — needs prod SSH or a spun-up compose stack. | Blocker | **Partial — pivoted to SQL-based backtest** in `backend/scripts/audit_estimator_changes.sql` (5 queries over prod Postgres). See §6. |
| F | **Sigma ceiling confirmed live.** bonds_detailed sigma distribution (n=1.19M): p05=0.061, p50=0.173, p95=0.449. With `max(0.005, min(0.03, sigma*0.5))`, every sigma ≥ 0.06 clamps to 3% — covers 95%+ of SA2s. Practical effect: ±1% → ±3% flat. Adaptive claim in commit `a94b46f` is theatre. | Cosmetic | Accepted — still a net improvement. See Finding B. |
| G | **CV-age fallback coverage is narrow.** Only 6 of 40+ TAs match `REVALUATION_DATES` keys via substring: Auckland (600K), Christchurch City (186K), Wellington City (84K), Tasman District (28K), South Taranaki (16K, fuzzy), Buller District (15K). ~570K properties across 25+ TAs fall to `cv_age=None`. Top missing by volume: Dunedin (57K), Lower Hutt (45K), New Plymouth (40K), Selwyn (37K), Queenstown-Lakes (34K). | High — scope gap | Rollout task #24 (wire `date_field` per-council) is the proper fix. |
| H | **imp_ratio age_proxy coverage = 98.8% of SA2s** have n ≥ 20 (1,582 of 1,601). Average SA2 imp_ratio distribution: p25=28.8%, p75=51.4%, IQR=22.7%. Of 1.52M eligible houses: 24.8% would get +2-6% uplift, 24.8% would get -8 to -3% discount. Balanced by construction. | Info | **Validated** — age_proxy will fire for roughly half of all house estimates with fresh CV. |
| I | **P0 #1 rollout blocked by data availability.** Probed 11 council ArcGIS endpoints: only Auckland (`LATESTVALUATIONDATE`) and KCDC (`Valuation_Date`) expose real per-property valuation date fields. Christchurch, Dunedin, Hutt (HCC), Waikato-shared (6 councils), Marlborough, PCC: no date fields. Tasman/Horizons (7 councils): only generic `DataLastUpdated`/`UpdatedDate_UTC` sync stamps — unsafe to use as valuation date. ECan-shared (8 Canterbury councils): `PublishDate`/`ProcessDate` are record-keeping, not valuation. Taranaki: `Valuation_Date` is string-typed but null in live samples. | Data-availability blocker | Wired KCDC (live); added `Hamilton City` + `Kapiti Coast District` to `REVALUATION_DATES` fallback. Remaining 22 councils truly lack a public date source — need QV licence or council paid feeds to go further. |
| J | **Snapshot plumbing done.** Each `rent_baselines[*]` entry now includes `sigma` (for client-side percentile calc) and `market_quartiles`. Hosted rent bands now use the sigma-derived inner pad (matches live advisor behaviour). `cv_age_months` was already surfaced via `snapshot.price_advisor.cv_age_months`. | Info | **Completed** — hosted reports have data parity with live endpoint minus the derived `percentile` (computed client-side). |
