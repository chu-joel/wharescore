...
| Improvements per room vs SA2 median | `price_advisor.py:367-426` | 2 | **Denominator asymmetry bug**: subject uses `beds + baths` (actual user input, e.g. 6 for 4-bed 2-bath); SA2 median uses hardcoded `typical_rooms = 3/4`. Ratio = `(imp/6) / (sa2/4)` = true_ratio × 0.67 → bigger homes systematically flagged "Below-average build", smaller homes "Above-average build". Fix: use consistent denominator on both sides, or replace this signal entirely with imp_ratio (P0 #4, now implemented). |
...
| `log_std_dev_weekly_rent` (sigma) | `rent_advisor.py:159` | 1 | Surfaced into baseline dict as `"sigma"` but **never read** downstream; `estimate_percentile` (`market.py:46`) exists but only called from `routers/market.py:240` |
...
| Improvements per room vs SA2 | `rent_advisor.py:777-849` | 2 | Same denominator-asymmetry bug as price advisor's equivalent block (subject uses `beds + baths`, SA2 median uses hardcoded `typical_rooms = 3/4`). |
...
| `cv_uncertainty` | — | 0 | **Not called in rent advisor**; `market_confidence_stars(bond_count, None, None)` at `rent_advisor.py:1311` always passes `None` for CV age |
...
| 2 | `cv_uncertainty` in rent advisor | `price_advisor:cv_uncertainty(cv_age_months)` | Fn exists | Pass `cv_age_months` into `market_confidence_stars` at `rent_advisor.py:1311`; also reduce quality-per-room adjustment magnitude when CV is old | Med — mostly better confidence score, not shift in band centre | S | P0 |
| 3 | Sigma (`log_std_dev_weekly_rent`) → band width & verdict | `bonds_detailed.log_std_dev_weekly_rent` | Yes, read into baseline | Replace fixed ±1% inner band with `median × (exp(±sigma) - 1)` clamped to [0.5%, 3%]; also call `estimate_percentile(your_rent, median, sigma)` and expose as `percentile` | Med — better-calibrated bands in thin and volatile SA2s | S | P0 |
