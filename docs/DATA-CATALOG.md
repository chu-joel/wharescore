...
| mbie_epb_history | ~5,935 | id (UUID PK), earthquake_rating, completion_deadline, geom, first_seen_at, last_seen_at, removed_at, has_been_removed, raw_json | `scripts/load_wellington_data.py::load_mbie_epb` via MBIE national register API `https://epbr.building.govt.nz/api/public/buildings` (paginated, no auth). UPSERT on id; `has_been_removed` mirrors MBIE's flag (primary removal signal), `removed_at` stamped on first observation of removal, cleared if MBIE un-removes. Sweep stamps removed_at on rows that vanish from the feed entirely. `raw_json` stores the full MBIE list payload (noticeType, hasPartPriority, image, etc.) so new fields don't need migrations. Migration 0057 added these columns and renamed the old `mbie_epb` table. Guard: loader raises if fetch returns <4000 rows. | `mbie_epb` **view** (rows WHERE removed_at IS NULL AND has_been_removed = FALSE) queried by `get_property_report()` → `hazards.epb_count_300m`, `hazards.epb_nearest`. Query `mbie_epb_history` directly for delisted-building insights. |
