...
| mbie_epb_history | ~5,813 | id (UUID PK), earthquake_rating, completion_deadline, construction_type, design_date, seismic_risk_area, region, geom, first_seen_at, last_seen_at, removed_at, has_been_removed, raw_json | `scripts/load_wellington_data.py::load_mbie_epb` via `https://epbr.building.govt.nz/api/public/buildings?export=all` (single ~9MB call, no auth). **Do NOT use the paginated `?page=N` variant — it's broken and returns the same first 20 rows regardless of params.** UPSERT on id; `raw_json` captures the full list payload (noticeType, hasPartPriority, territorialAuthority, legalDescription, taReference, noticeNumber, etc.) for future field extraction without a schema change. Removal tracking: `export=all` only returns active buildings, so the backstop sweep (last_seen_at < load_started_at → stamp removed_at) is the primary removal signal. MBIE does NOT currently expose historical delistings via any JSON endpoint we've found — the UI shows them but no public API does. Migration 0057 added the soft-delete columns and renamed the old `mbie_epb` table. Loader guard: raises if fetch returns <4000 rows. | `mbie_epb` **view** (rows WHERE removed_at IS NULL AND has_been_removed = FALSE) queried by `get_property_report()` → `hazards.epb_count_300m`, `hazards.epb_nearest`. Query `mbie_epb_history` directly for delisted-building insights (will accumulate over time as buildings drop out of the feed). |
