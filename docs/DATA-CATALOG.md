...
| Key | Target table |
|-----|-------------|
| auckland_flood | flood_hazard (hazard_type='1%', depth-tiered ranking — authoritative 1% AEP / 1-in-100yr). Endpoint: AC `Flood_Prone_Areas` FeatureServer/0. Tagging is set in `data_loader.py::load_auckland_flood`. **First loader to use the upsert path** (`_load_council_arcgis_upsert`) with `stable_id_field="FPA_ID"` — per-feature diffs land in `data_change_log` (no DELETE-then-INSERT window, change history retained). |
