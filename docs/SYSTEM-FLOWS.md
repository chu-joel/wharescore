...
Builds a time-horizon view of coastal / sea-level-rise exposure (tier: `happens_now` / `within_30_years` / `longer_term` / `not_applicable`). Emitted by `backend/app/services/coastal_timeline.py::build_coastal_exposure(report)`, stored on `snapshot.coastal`, rendered by `HostedCoastalTimeline.tsx`.

### Tier classification (MVP, pre-SeaRise-data)

Until per-point NZ SeaRise + LINZ coastline + NIWA storm-tide polygons land, the tier is derived from existing hazard fields and SRTM terrain elevation:

| Tier | Trigger | UI |
|---|---|---|
| `happens_now` | Elevation ≤3m AND (coastal_inundation_ranking OR tsunami_zone_class ≥3) OR (elevation ≤3m AND a council coastal layer fires) | Red border, +12 Hazards chip |
| `within_30_years` | Elevation ≤5m AND (council layer OR inundation OR tsunami ≥2); OR elevation ≤10m AND (council layer OR tsunami ≥1) | Amber, +8 |
| `longer_term` | Any council coastal layer fires OR tsunami ≥1 (but elevation and other flags don't escalate) | Blue, +1 |
| `not_applicable` | Inland property with no coastal flags | Section hidden entirely |

### Renter vs buyer narratives

`build_coastal_exposure` emits `narrative` (buyer-framed, insurance repricing) and optionally `narrative_renter` (contents-insurance + evacuation framing, no building-insurance talk). Renters only see `happens_now` tier — everything else is hidden for them in the timeline.

### Score interplay (double-count prevention)

The timeline carries a `score_impact.delta` intended for the Hazards bucket, cap 15 points. When a council coastal layer is already firing (`coastal_erosion`, `coastal_erosion_nearby`, `coastal_inundation_ranking`, `council_coastal_erosion`), the delta is **halved** to avoid double-counting against the existing `coastal_erosion` (0.08) and `coastal_erosion_council` (0.08) weights in `risk_score.py`. 

**NOT YET SHIPPED:** feeding `score_impact.delta` into `risk_score.py`. Today it is a cosmetic chip. Phase D replaces the two legacy weights with a single unified coastal indicator driven by the timeline delta.

### Dedupe with HostedHazardAdvice

`HostedHazardAdvice.tsx` detects `snapshot.coastal != null && tier !== 'not_applicable'` and suppresses its internal `coastal-erosion` + `coastal-inundation` blocks so the story is told once.

### Data-source roadmap

| What | Source | Status |
|---|---|---|
| Per-point SLR projections (2km spacing) | NZ SeaRise (searise.nz) | Not loaded. Hardcoded `NATIONAL_SLR` averages used as fallback in `coastal_timeline.py`. |
| Coast distance | LINZ NZ coastline polylines | Not loaded. `coast_distance_m` returned as null. |
| 100yr storm-tide polygons | NIWA Extreme Coastal Flood Maps | Not loaded. `storm_tide_100yr_distance_m` returned as null. |
| Ground elevation | SRTM DEM | Loaded. Read from `report.terrain.elevation_m`. |
| Vertical land motion | NZ SeaRise VLM per point | Not loaded. `vlm_mm_yr` returned as null. |
