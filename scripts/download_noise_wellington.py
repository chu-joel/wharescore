"""Download NZTA Road Noise Contours for Wellington region via ArcGIS REST API."""
import json
...
NDJSON_FILE = os.path.join(OUTPUT_DIR, "wellington-noise-features.ndjson")
OUTPUT = os.path.join(OUTPUT_DIR, "wellington-noise-contours.geojson")
PAGE_SIZE = 2000
TOTAL_EXPECTED = 19517

# Wellington region bounding box
BBOX = "174.6,-41.6,175.5,-40.85"
...
def fetch_page(offset):
    params = (
        ...
        f"&outFields=LAeq24h&outSR=4326&f=geojson"
...
def main():
    ...
    # Cleanup NDJSON temp file
    os.remove(NDJSON_FILE)
    print("Cleaned up temp NDJSON file.", flush=True)
