# backend/app/services/gdc_rates.py
"""
Gisborne District Council ArcGIS property data client.
Uses the same ArcGIS Online FeatureServer as Upper Hutt (rating_valuation)
which contains Gisborne property data.
...
async def fetch_gdc_rates(address: str, conn=None) -> dict | None:
    try:
        ...
        url = f"{GDC_URL}?{urllib.parse.urlencode(params)}"
        ...
        if rates:
            levy_breakdown.append({
                ...
                "items": [{"description": "Gisborne District Council Rates", "ratesAmount": rates}],
                "subtotal": rates,
            })
        ...
        logger.warning(f"GDC ArcGIS error for {address}: {e}")
        return None
...
async def _fetch_json(url: str, timeout: int = 8) -> dict | None:
    try:
        ...
        logger.warning(f"GDC fetch failed: {e}")
        return None
