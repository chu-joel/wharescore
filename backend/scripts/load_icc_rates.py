#!/usr/bin/env python3
"""
Bulk loader for Invercargill City Council property valuations.

Fetches all properties from the ICC ArcGIS MapServer and inserts them
into the council_valuations table. Uses pagination (2000 records/page).

~33K properties, takes about 1-2 minutes.

Usage:
    cd backend
    python scripts/load_icc_rates.py [--dry-run] [--clear]
"""
...
def fetch_page(offset: int) -> list[dict]:
    """Fetch a page of properties from ICC ArcGIS."""
    params = {
        "where": "1=1",
    ...
    resp = requests.get(ICC_QUERY_URL, params=params, timeout=30,
...
def main(args):
    ...
    if args.clear:
        logger.info("Clearing existing ICC data from council_valuations...")
        conn.execute("DELETE FROM council_valuations WHERE council = 'ICC'")
        conn.commit()
        logger.info("Cleared.")
    ...
    if args.dry_run:
        ...
        for f in all_features[:5]:
            a = f["attributes"]
            print(f"  {a['ADDRESS']:40s} CV=${a['CAPITAL']:>10,}  LV=${a['LAND']:>10,}")
    ...
    for f in unique_features:
        ...
        full_address = f"{address}, Invercargill" if address else None

        house = a.get("HOUSE")
        street_number = str(int(house)) if house else None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Load ICC property valuations")
    parser.add_argument("--dry-run", action="store_true", help="Fetch but don't insert")
    parser.add_argument("--clear", action="store_true", help="Clear existing ICC data first")
