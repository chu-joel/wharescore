-- 0030: Split transit travel times into AM and PM peak windows
--
-- Previously get_transit_data() returned travel times without filtering
-- by peak_window, so AM and PM were mixed. Now returns separate
-- transit_travel_times (AM peak) and transit_travel_times_pm (PM peak).
-- Also filters AM queries to peak_window='am' for accurate morning commute times.

-- Re-create the function with AM/PM split (same signature, CREATE OR REPLACE)
...
BEGIN
  ...
  -- AM Travel times: try metlink first, then AT, then regional transit_stops
  ...
  -- PM Travel times (same cascade: metlink → AT → regional)
  SELECT jsonb_agg(jsonb_build_object(
    'destination', best.destination,
    'minutes', best.min_minutes,
    'routes', best.route_names
  ) ORDER BY best.min_minutes)
  INTO travel_times_pm
  FROM (
    SELECT DISTINCT ON (ttt.destination)
      ttt.destination, ttt.min_minutes, ttt.route_names
    FROM metlink_stops ms
  ...
  IF travel_times_pm IS NULL THEN
    SELECT jsonb_agg(jsonb_build_object(
      'destination', best.destination,
      'minutes', best.min_minutes,
      'routes', best.route_names
  ...
  RETURN jsonb_build_object(
    ...
    'transit_travel_times_pm', travel_times_pm,
