-- 0006_national_report_function.sql
-- Make get_property_report() work nationally:
-- 1. CBD distance uses region-aware coordinates (Auckland, Wellington, Christchurch, etc.)
-- 2. Transit queries check both Metlink (Wellington) and AT (Auckland) tables
-- 3. Mode breakdown works for both regions
...
BEGIN
  ...
  result := jsonb_build_object(
    ...
    'hazards', (
      ...
      LEFT JOIN LATERAL (
        SELECT wind_speed_zone FROM wind_zones
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) wz ON true
      LEFT JOIN LATERAL (
      ...
      LEFT JOIN LATERAL (
        SELECT risk_level, days FROM wildfire_risk
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) wf ON true
      LEFT JOIN LATERAL (
      ...
      LEFT JOIN LATERAL (
        SELECT site_name, pm10_avg, pm10_trend
        FROM air_quality_sites
        WHERE geom && ST_Expand(addr.geom, 0.1)
        ORDER BY geom <-> addr.geom LIMIT 1
      ) aq ON true
      LEFT JOIN LATERAL (
        SELECT site_name, ecoli_band, mci_score
        FROM water_quality_sites
        WHERE geom && ST_Expand(addr.geom, 0.05)
        ORDER BY geom <-> addr.geom LIMIT 1
      ) wq ON true
      LEFT JOIN LATERAL (
        SELECT sensitivity, jsonb_build_object('csi', csi_class, 'geomorphology', geomorphology) AS detail
        FROM coastal_erosion_national
      ...
      LEFT JOIN LATERAL (
        SELECT
          (cp.temp_mid_2041 - cg.annual_mean_temp) AS temp_change,
          cp.precip_mid_2041 AS rainfall_change
        FROM climate_grid cg
        LEFT JOIN climate_projections cp ON cp.grid_id = cg.id
    ...
    'liveability', (
      ...
      LEFT JOIN LATERAL (
        SELECT
          c.area_unit AS crime_area_unit,
          c.victimisations AS crime_victimisations,
          c.percentile_rank,
          city_agg.median_vics AS city_median_vics,
          city_agg.total_vics AS city_total_vics,
          city_agg.area_count AS city_area_count
        FROM crime c
        LEFT JOIN LATERAL (
          SELECT
            percentile_cont(0.5) WITHIN GROUP (ORDER BY c2.victimisations)::int AS median_vics,
            SUM(c2.victimisations)::int AS total_vics,
            COUNT(*)::int AS area_count
          FROM crime c2
          WHERE c2.ta = c.ta
        ) city_agg ON true
        WHERE (
          c.area_unit ILIKE '%' || coalesce(v_sa2_name, '') || '%'
          OR c.area_unit ILIKE '%' || coalesce(addr.suburb_locality, '') || '%'
        )
        ORDER BY c.victimisations DESC LIMIT 1
      ) cd ON true
      -- Schools (1.5km)
      LEFT JOIN LATERAL (
        SELECT jsonb_agg(jsonb_build_object(
          'name', s.name, 'type', s.school_type,
          'decile', s.decile, 'roll', s.roll,
      ...
      -- Transit stop list (nearest 10)
      LEFT JOIN LATERAL (
        SELECT jsonb_agg(jsonb_build_object(
          'name', tsl.stop_name, 'distance_m', round(tsl.dist::numeric),
          'mode', CASE tsl.mode_type WHEN 'bus' THEN 'bus' WHEN 'train' THEN 'train' WHEN 'ferry' THEN 'ferry' ELSE 'bus' END
        ) ORDER BY tsl.dist) AS stops
        FROM (
          SELECT stop_name, mode_type, ST_Distance(geom::geography, addr.geom::geography) AS dist
          FROM transit_stops
          WHERE geom && ST_Expand(addr.geom, 0.005)
            AND ST_DWithin(geom::geography, addr.geom::geography, 400)
      ...
      LEFT JOIN LATERAL (
        ...
        FROM transit_stops
        WHERE mode_type = 'train'
          AND geom && ST_Expand(addr.geom, 0.05)
          AND ST_DWithin(geom::geography, addr.geom::geography, 5000)
        ORDER BY geom <-> addr.geom LIMIT 1
      ) tr ON true
      -- Crashes within 300m (last 5 years)
      LEFT JOIN LATERAL (
        SELECT
          COUNT(*) FILTER (WHERE severity = 'Serious Injury')::int AS serious_count,
          COUNT(*) FILTER (WHERE severity = 'Fatal')::int AS fatal_count,
