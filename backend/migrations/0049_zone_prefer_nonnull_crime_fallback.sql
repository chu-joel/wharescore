-- 0049_zone_prefer_nonnull_crime_fallback.sql
-- Two targeted fixes over 0033_cap_nearest_train.sql:
--
-- 1. district_plan_zones lookup now ORDERs BY "zone_name IS NULL" before
--    LIMIT 1, so when the table has overlapping rows at one point (some with
--    null zone_name, some populated) the populated row wins. Fixes
--    Christchurch Cathedral Square where the CCC FeatureServer layer has a
--    null placeholder polygon alongside the real 'City centre zone' row.
--
-- 2. crime_percentile now falls back to mv_crime_ta_ranked.ta_percentile when
--    the fuzzy area_unit match fails, instead of returning NULL. Also stops
--    returning the whole-TA victimisations count when no area-unit match is
--    found — we use the TA median-per-AU instead so users see a "typical
--    neighbourhood" number, not a 95K city-wide total. Fixes Christchurch
--    Central where sa2_name 'Christchurch Central' has no similarity to the
--    police AU name 'Cathedral Square'.
