-- 0051_crime_area_unit_fallback_and_zone_category_sort.sql
-- Two small fixes over 0049 surfaced by the 4-city audit:
--
-- 1. liveability.crime_area_unit was NULL when the mv_crime_density fuzzy
--    match failed (e.g. CHC/Cathedral Square where 'Christchurch Central'
--    didn't find any area_unit with similarity > 0.3). The fallback branch
--    still populated victimisations + percentile from the TA totals but
--    left area_unit NULL, so the report had no label to display. Now we
--    COALESCE(best_au.area_unit, v_sa2_name) so users always see the SA2
--    name when no specific police area_unit matches.
--
-- 2. district_plan_zones ORDER BY now also prefers rows with a populated
--    category. QLDC and CHC loaders don't always populate category, so
--    when two overlapping polygons both match, the one with a real
--    category label wins over the one that has zone_name but null
--    category. The existing preference for non-null zone_name/code is
