-- 0054_flood_nearest_m.sql
--
-- Adds `flood_nearest_m` to the hazards JSON so the frontend can flag
-- properties that are CLOSE TO a mapped flood zone but not directly in one.
-- "In a flood zone" is already detected by the combination of flood_zone,
-- flood_extent_label, flood_extent_aep, and wcc_flood_type; that covers the
-- polygon-intersect case. This adds proximity: the nearest flood polygon
-- across flood_zones (GWRC national), flood_hazard (WCC + regional council
-- overlays), and flood_extent (GWRC AEP extents) within a 500m search radius.
--
-- Frontend treats:
--   - flood_nearest_m IS NULL or > 100 and not in zone  → no flag
--   - flood_nearest_m <= 100 and not in zone            → "within Nm of flood zone"
--   - in zone (any of the 4 flags)                      → "in a flood zone"
--
-- 500m ceiling avoids blowing out spatial scans on inland properties that
-- have no flood data nearby; 100m is the UI threshold (about a street width).
--
-- Preserves all behaviour from 0051 (crime_area_unit fallback, district plan
-- zone_category sort). Only the hazards section changes.
  ...
  result := jsonb_build_object(
    ...
    'hazards', (
      SELECT jsonb_build_object(
        ...
        -- Nearest flood polygon within 500m across all three flood tables.
        -- NULL when nothing is within 500m. 0 when the property is inside a
        -- polygon. Frontend flags <=100 as "close to a flood zone".
        'flood_nearest_m', flood_near.dist_m,
      ...
      -- Nearest flood polygon within 500m across the three flood tables.
      -- We take the minimum distance so that whichever layer is closest wins.
      -- Distances computed in metres via ::geography to get accurate NZ
      -- projection; bounding-box pre-filter (0.006 ~= 500m at NZ latitude)
      -- keeps the scan cheap via the existing GIST indexes.
      LEFT JOIN LATERAL (
        SELECT MIN(dist_m)::double precision AS dist_m FROM (
          SELECT ST_Distance(geom::geography, addr.geom::geography) AS dist_m
            FROM flood_hazard
    ...
    'liveability', (
      ...
      -- Crime: fuzzy match SA2 name / suburb to area_unit, fall back to
      -- (a) the TA median victimisations + TA percentile (from mv_crime_ta_ranked,
      --     created by 0045) when the fuzzy match fails, rather than the whole-TA
      --     victimisations total which produced misleadingly huge numbers like
      --     95,307 for "Christchurch Central".
      LEFT JOIN LATERAL (
        SELECT
          coalesce(
            best_au.victimisations_3yr,
            ta.median_victimisations_per_au  -- "typical neighbourhood", not whole TA total
          ) AS crime_victimisations,
          COALESCE(best_au.area_unit, v_sa2_name) AS crime_area_unit,
          coalesce(best_au.percentile_rank, tar.ta_percentile / 100.0) AS percentile_rank,
        ...
        FROM mv_crime_ta ta
        LEFT JOIN mv_crime_ta_ranked tar ON tar.ta = ta.ta
        LEFT JOIN LATERAL (
    ...
    'planning', (
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE ST_Intersects(geom, addr.geom)
        ORDER BY
          CASE WHEN zone_name IS NULL THEN 1 ELSE 0 END,
