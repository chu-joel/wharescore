-- 0005_update_report_function_national.sql
-- Updated get_property_report() using renamed table names from 0004.
-- JSON output keys are IDENTICAL to the previous version — no downstream breakage.
-- Also adds GNS active faults data to the hazards section.
  ...
  result := jsonb_build_object(
    ...
    'hazards', (
      SELECT jsonb_build_object(
        ...
        -- Regional hazard detail (was Wellington-specific, now works for any council)
        'earthquake_hazard_index', reg_eq.chi,
        'earthquake_hazard_grade', reg_eq.chi_grade,
        'ground_shaking_zone', reg_gs.zone,
        'ground_shaking_severity', reg_gs.severity,
        'gwrc_liquefaction', reg_liq.liquefaction,
        'gwrc_liquefaction_geology', reg_liq.simplified,
        'gwrc_slope_severity', reg_sf.severity,
        'fault_zone_name', fz_dp.name,
        'fault_zone_ranking', fz_dp.hazard_ranking,
        'wcc_flood_type', fh_dp.hazard_type,
        'wcc_flood_ranking', fh_dp.hazard_ranking,
        'wcc_tsunami_return_period', th_dp.return_period,
        'wcc_tsunami_ranking', th_dp.hazard_ranking,
        ...
        'active_fault_nearest', af.nearest_fault,
        'fault_avoidance_zone', faz.in_faz,
        -- Coastal hazards
        'coastal_elevation_m', coast_elev.elevation_m,
        ...
        -- Landslide susceptibility (council data — Auckland etc.)
        'landslide_susceptibility_rating', ls_susc.rating,
        'landslide_susceptibility_type', ls_susc.susc_type,
        -- Overland flow path
        'on_overland_flow_path', coalesce(ofp.on_flow_path, false),
        -- Coastal erosion
        'coastal_erosion_exposure', ce_data.exposure,
        'coastal_erosion_timeframe', ce_data.timeframe,
        -- Aircraft noise overlay
        'aircraft_noise_dba', anoise.noise_level_dba,
        'aircraft_noise_category', anoise.noise_category,
        -- Geotechnical reports nearby
        'geotech_count_500m', geotech.cnt,
        'geotech_nearest_hazard', geotech.nearest_hazard
      )
      FROM (SELECT 1) x
      LEFT JOIN LATERAL (
      ...
      LEFT JOIN LATERAL (
        SELECT exposure FROM coastal_erosion
        WHERE ST_DWithin(geom::geography, addr.geom::geography, 2000)
      ...
      -- Regional: earthquake hazard index (renamed from gwrc_earthquake_hazard)
      LEFT JOIN LATERAL (
        SELECT chi, chi_hazard_grade AS chi_grade
        FROM earthquake_hazard
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) reg_eq ON true
      -- Regional: ground shaking amplification (renamed from gwrc_ground_shaking)
      LEFT JOIN LATERAL (
        SELECT zone, severity FROM ground_shaking
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) reg_gs ON true
      -- Regional: liquefaction detail (renamed from gwrc_liquefaction)
      LEFT JOIN LATERAL (
        SELECT liquefaction, simplified FROM liquefaction_detail
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) reg_liq ON true
      -- Regional: slope failure (renamed from gwrc_slope_failure)
      LEFT JOIN LATERAL (
        SELECT severity FROM slope_failure
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) reg_sf ON true
      -- District plan: fault zones (renamed from wcc_fault_zones)
      LEFT JOIN LATERAL (
        SELECT name, hazard_ranking FROM fault_zones
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) fz_dp ON true
      -- District plan: flood hazard (renamed from wcc_flood_hazard)
      LEFT JOIN LATERAL (
      ...
      -- District plan: tsunami tiers (renamed from wcc_tsunami_hazard)
      LEFT JOIN LATERAL (
      ...
      -- Building solar radiation (still wcc_solar_radiation table name)
      LEFT JOIN LATERAL (
      ...
      -- GNS Active Faults: nearest fault within 2km
      LEFT JOIN LATERAL (
        SELECT jsonb_build_object(
          'name', fault_name,
          'class', fault_class,
      ...
      -- GNS Fault Avoidance Zone: is property inside?
      LEFT JOIN LATERAL (
        SELECT jsonb_build_object(
      ...
      -- Erosion prone land (regional)
      LEFT JOIN LATERAL (
        SELECT TRUE AS on_erosion_prone, min_angle AS erosion_min_angle
        FROM erosion_prone_land
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) epl ON true
      -- Landslide susceptibility (council polygons — Auckland etc.)
      LEFT JOIN LATERAL (
        SELECT accuracy AS rating, type AS susc_type
        FROM landslide_susceptibility
        WHERE ST_Intersects(geom, addr.geom)
        ORDER BY CASE LOWER(accuracy)
          WHEN 'very high' THEN 1 WHEN 'high' THEN 2 WHEN 'moderate' THEN 3
          WHEN 'medium' THEN 3 WHEN 'low' THEN 4 WHEN 'very low' THEN 5 ELSE 6 END
        LIMIT 1
      ) ls_susc ON true
      -- Overland flow path (within 30m)
      LEFT JOIN LATERAL (
        SELECT TRUE AS on_flow_path
        FROM overland_flow_paths
        WHERE geom && ST_Expand(addr.geom, 0.0004)
          AND ST_DWithin(geom::geography, addr.geom::geography, 30)
        LIMIT 1
      ) ofp ON true
      -- Coastal erosion
      LEFT JOIN LATERAL (
        SELECT exposure, timeframe
        FROM coastal_erosion
        WHERE ST_Intersects(geom, addr.geom)
        ORDER BY CASE LOWER(exposure)
          WHEN 'very high' THEN 1 WHEN 'high' THEN 2 WHEN 'moderate' THEN 3
      ...
      -- Aircraft noise overlay
      LEFT JOIN LATERAL (
        SELECT noise_level_dba, noise_category
        FROM aircraft_noise_overlay
        WHERE ST_Intersects(geom, addr.geom)
        ORDER BY noise_level_dba DESC LIMIT 1
      ) anoise ON true
      -- Geotechnical reports within 500m
      LEFT JOIN LATERAL (
        SELECT COUNT(*)::int AS cnt,
               (SELECT hazard FROM geotechnical_reports g2
                WHERE g2.geom && ST_Expand(addr.geom, 0.005)
                  AND ST_DWithin(g2.geom::geography, addr.geom::geography, 500)
                  AND g2.hazard IS NOT NULL
                ORDER BY g2.geom <-> addr.geom LIMIT 1) AS nearest_hazard
    ...
    'liveability', (
      SELECT jsonb_build_object(
        ...
        -- Transit travel times to key destinations (AM + PM peak)
        'transit_travel_times', tt.times,
        'transit_travel_times_pm', tt_pm.times,
      ...
      -- Transit travel times: AM peak (7–9 AM) — best time to each destination within 400m
      LEFT JOIN LATERAL (
        SELECT jsonb_agg(jsonb_build_object(
      ...
      -- Transit travel times: PM peak (4:30–6:30 PM)
      LEFT JOIN LATERAL (
        SELECT jsonb_agg(jsonb_build_object(
          'destination', best.destination,
          'minutes', best.min_minutes,
    ...
    'planning', (
      SELECT jsonb_build_object(
        ...
        'heritage_overlay_name', hho.heritage_name,
        'heritage_overlay_type', hho.heritage_type,
        -- Special character area
        'in_special_character_area', coalesce(sca.in_sca, false),
        'special_character_name', sca.sca_name,
        -- Notable trees within 50m
        'notable_tree_count_50m', nt.cnt,
        'notable_tree_nearest', nt.nearest_name,
        -- Significant ecological area
        'in_ecological_area', coalesce(sea.in_sea, false),
        'ecological_area_name', sea.sea_name,
        -- Parks within 500m
        'park_count_500m', pk.cnt,
        'nearest_park_name', pk.nearest_name,
        'nearest_park_distance_m', pk.nearest_dist
      )
      FROM (SELECT 1) x
      LEFT JOIN LATERAL (
      ...
      -- Historic heritage overlay (Auckland etc.)
      LEFT JOIN LATERAL (
        SELECT name AS heritage_name, heritage_type
        FROM historic_heritage_overlay
        WHERE ST_Intersects(geom, addr.geom) LIMIT 1
      ) hho ON true
      -- Special character area
      LEFT JOIN LATERAL (
        SELECT TRUE AS in_sca, name AS sca_name
      ...
      LEFT JOIN LATERAL (
        SELECT COUNT(*)::int AS cnt,
               (SELECT name FROM notable_trees nt2
                WHERE nt2.geom && ST_Expand(addr.geom, 0.0006)
                  AND ST_DWithin(nt2.geom::geography, addr.geom::geography, 50)
                ORDER BY nt2.geom <-> addr.geom LIMIT 1) AS nearest_name
        FROM notable_trees
      ...
      -- Significant ecological area
      LEFT JOIN LATERAL (
        SELECT TRUE AS in_sea, name AS sea_name
      ...
      LEFT JOIN LATERAL (
        SELECT COUNT(*)::int AS cnt,
               (SELECT site_name FROM park_extents pk2
                WHERE pk2.geom && ST_Expand(addr.geom, 0.005)
                  AND ST_DWithin(pk2.geom::geography, addr.geom::geography, 500)
                ORDER BY pk2.geom <-> addr.geom LIMIT 1) AS nearest_name,
               (SELECT round(ST_Distance(pk3.geom::geography, addr.geom::geography)::numeric)
                FROM park_extents pk3
                WHERE pk3.geom && ST_Expand(addr.geom, 0.005)
                  AND ST_DWithin(pk3.geom::geography, addr.geom::geography, 500)
                ORDER BY pk3.geom <-> addr.geom LIMIT 1) AS nearest_dist
        FROM park_extents
        WHERE geom && ST_Expand(addr.geom, 0.005)
          AND ST_DWithin(geom::geography, addr.geom::geography, 500)
      ) pk ON true
    ...
    'market', (
      ...
      LEFT JOIN LATERAL (
        ...
        FROM rbnz_housing ORDER BY quarter_end DESC LIMIT 1
      ) hpi ON true
    )
  );
